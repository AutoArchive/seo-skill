...
request. The calling skill supplies domain-specific implementation and
post-merge acceptance requirements.
...
3. Define one coherent result, smallest compatible diff, preserved behavior,
   authoritative checks, risk, rollback, and post-merge handoff.
...
3. Inspect all generated output, test results, CI logs or summaries, dependency
   and submodule movement, and review comments relevant to the final head.
...
5. Check correctness, regression risk, error paths, public or private data
   ...
   and rollback.
6. Re-run or independently inspect the acceptance evidence most likely to catch
   a false positive.
...
The caller handles production deployment and public verification. Any corrective
or closeout change is a new pull request and repeats this entire checklist.

## Prohibited shortcuts

- direct automated push to the default branch;
- force-push, reset, stash, or overwrite of unrelated work;
- draft PR, issue, local commit, preview, workflow URL, or HTTP status used as a
  substitute for completed delivery;
